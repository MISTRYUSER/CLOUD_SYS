...
#define TIMEOUT_SECONDS 5 // 超时时间，60秒
...
char home[PATH_MAX] = "/home/xuewentao/project/test_dir/";
...
client_state_t* get_client_state(int netfd) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (!client_states[i].is_alive) {
            ...
            client_states[i].current_pwd_id = 1;
    ...
    while (1) {
        ...
        for (int i = 0; i < ready_num; ++i) {
            ...
            if (fd == exit_pipe[READ]) {
                ...
                if ((type >= TLV_TYPE_CD && type <= TLV_TYPE_USERCANCEL) || type == TLV_TYPE_REMOVE) {
                    // 短命令在主线程处理
                    solve_command(netfd, &tlv_packet, mysql,state);
