...
extern  char current_username[50]; // 用于保存当前处理的用户名
...
#define MAX_CHUNK_SIZE 4096  // 每个数据块的最大大小
/*
 * TLV 协议数据包结构:
...
typedef struct {
    ...
    uint32_t chunk_index;   // 当前块索引
    uint32_t total_chunks;  // 总块数
    uint8_t is_last;        // 是否为最后一块
    char data[MAX_CHUNK_SIZE]; // 块数据
} chunk_data_t;
...
int file_gets(MYSQL *mysql, const char *username, const char *filename,
              const char *path, char *response, size_t res_size, const char *userpath);
 ...
 */
int compute_sha1(const char* path, const char *sha1_res);
/** @} */
